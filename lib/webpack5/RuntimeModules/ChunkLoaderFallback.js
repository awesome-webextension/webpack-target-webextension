...
module.exports = function ChunkLoaderFallbackRuntimeModule(webpack) {
  ...
  class ChunkLoaderFallbackRuntimeModule extends RuntimeModule {
    ...
    generate() {
      ...
      const optionalChain = this.compilation.outputOptions.environment.optionalChaining
      const _const = this.compilation.outputOptions.environment.const ? 'const' : 'var'
      const _let = this.compilation.outputOptions.environment.const ? 'let' : 'var'
