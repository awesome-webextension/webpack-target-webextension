...
module.exports = class ManifestV3ServiceWorkerPlugin {
  ...
  apply(compiler) {
    ...
    compiler.hooks.entryOption.tap(ManifestV3ServiceWorkerPlugin.name, (context, entries) => {
...
Set serviceWorker.noWarningDynamicEntry to true to disable this warning.
    ...
    if (this.options.eagerChunkLoading !== false) {
      compiler.hooks.compilation.tap(ManifestV3ServiceWorkerPlugin.name, (compilation) => {
        compilation.hooks.afterOptimizeChunkIds.tap(ManifestV3ServiceWorkerPlugin.name, () => {
