...
// Content script
const ContentScriptEntryPlugin = require('./ContentScript.js')
...
module.exports = class WebExtensionPlugin {
   ...
   */
  apply(compiler) {
    const { background, contentScript } = this.options
    if (background?.serviceWorkerEntry) new ServiceWorkerEntryPlugin(background).apply(compiler)
    if (contentScript) new ContentScriptEntryPlugin(contentScript).apply(compiler)
