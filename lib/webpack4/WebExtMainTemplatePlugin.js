const WebExtHotRuntime = require('./WebExtHot.runtime')
...
class WebExtMainTemplatePlugin {
  ...
  apply(mainTemplate) {
    const { SyncWaterfallHook, Template } = this.imports
    ...
    mainTemplate.hooks.localVars.tap('WebExtMainTemplatePlugin', (source, chunk, hash) => {
      const extraCode = []
      ...
      if (needChunkOnDemandLoadingCode(chunk)) {
        extraCode.push(
          ...
          Template.indent([
            ...
            Template.indent([
              '? new Promise(resolve => {',
              Template.indent(['chrome.runtime.sendMessage(message, resolve);']),
              '})',
          ...
          Template.indent([
            ...
            `var scriptSrcPath = publicPath + ${getScriptSrcPath(hash, chunk, 'chunkId')};`,
    ...
    mainTemplate.hooks.jsonpScript.tap('WebExtMainTemplatePlugin', (_, chunk, hash) => {
    ...
    mainTemplate.hooks.linkPreload.tap('WebExtMainTemplatePlugin', (_, chunk, hash) => {
      return Template.asString(['var link = webextScriptSrc(chunkId);'])
    })
    mainTemplate.hooks.linkPrefetch.tap('WebExtMainTemplatePlugin', (_, chunk, hash) => {
      return Template.asString(['var link = webextScriptSrc(chunkId);'])
    })
    mainTemplate.hooks.requireEnsure.tap('WebExtMainTemplatePlugin load', (source, chunk, hash) => {
      return Template.asString([
        source,
        '',
    ...
    mainTemplate.hooks.requireExtensions.tap('WebExtMainTemplatePlugin', (source, chunk) => {
    ...
    mainTemplate.hooks.bootstrap.tap('WebExtMainTemplatePlugin', (source, chunk, hash) => {
      if (needChunkLoadingCode(chunk)) {
    ...
    mainTemplate.hooks.beforeStartup.tap('WebExtMainTemplatePlugin', (source, chunk, hash) => {
      if (needChunkLoadingCode(chunk)) {
        var jsonpFunction = mainTemplate.outputOptions.jsonpFunction
        var globalObject = mainTemplate.outputOptions.globalObject
        return Template.asString([
          `var jsonpArray = ${globalObject}[${JSON.stringify(jsonpFunction)}] = ${globalObject}[${JSON.stringify(
            jsonpFunction
          )}] || [];`,
    ...
    mainTemplate.hooks.afterStartup.tap('WebExtMainTemplatePlugin', (source, chunk, hash) => {
    ...
    mainTemplate.hooks.startup.tap('WebExtMainTemplatePlugin', (source, chunk, hash) => {
    ...
    mainTemplate.hooks.hotBootstrap.tap('WebExtMainTemplatePlugin', (source, chunk, hash) => {
      ...
      const runtimeSource = Template.getFunctionContent(WebExtHotRuntime)
        .replace(/\/\/\$semicolon/g, ';')
